...
background: linear-gradient(158deg, rgba(255,32,32,1) 0%, rgba(172,10,10,1) 100%);"><i class="bi bi-box-arrow-right"></i><span> Logout</span></a>
    ...
    <div class="main-content" id="mainContent">
        <!-- Header -->
        <div class="header">
            <div class="logo" style="font-size:22px;">ระบบจัดการคอร์สติว</div>
            ...
            <div class="card">
                ...
                <?php
                    ...
                    if ($result->num_rows > 0) {
                        ...
                        echo "<tr>";
                        echo "<th>รูปภาพ</th>";
                        echo "<th class='td_th_namewidth'>ชื่อคอร์สติว</th>";
                        ...
                        while($row = $result->fetch_assoc()) {
                            ...
                            echo "<td><a href='meetingroom_edit.php?id=" . $row["id_course"] . "' class='edit-button'>แก้ไข</a></td>";
                            // echo "<td><a href='meetingroom_delete.php?id=" . $row["id_room"] . "' class='delete-button'>ลบ</a></td>";
                            echo "<td><a href='#' onclick='confirmDelete(" . $row["id_course"] . ")' class='delete-button'>ลบ</a></td>";
                            echo "</tr>";
                        }
                        echo "</tbody>";
                        echo "</table>";
                    } else {
                        echo "ไม่มีคอร์สที่จะแสดง";
                    }
                    $stmt->close();
                    $conn->close();
                    ?>
...
function confirmDelete(id) {
        Swal.fire({
            title: 'ยืนยันการลบ?',
            text: "คุณต้องการลบคอร์สติวนี้ใช่หรือไม่?",
            icon: 'warning',
