...
<head>
    ...
    <style>
        * {
        ...
        .status-unknown {
            color: #757575; /* สีเทา สำหรับสถานะที่ไม่รู้จัก */
        }
    </style>
</head>
<body>
...
background: linear-gradient(158deg, rgba(255,32,32,1) 0%, rgba(172,10,10,1) 100%);"><i class="bi bi-box-arrow-right"></i><span> Logout</span></a>
    ...
    <div class="main-content" id="mainContent">
        <!-- Header -->
        <div class="header">
            <div class="logo" style="font-size:22px;">ระบบจัดการการลงทะเบียนคอร์สติว</div>
            <div class="notification">.</div>
        </div>

        <!-- Dashboard Content -->
        <div class="section">
            <h3>สรุปการลงทะเบียนคอร์สติว</h3>
            <div class="card">
                <p>แสดงจำนวนการลงทะเบียนคอร์สติว</p>
                ...
                <?php
                    ...
                    $sql = "SELECT *
                            ...
                            ORDER BY register_course.id_register_course DESC";
                    ...
                    if ($result->num_rows > 0) {
                        ...
                        echo "<th>ผู้ลงทะเบียน</th>";
                        echo "<th>วันที่เริ่มเรียน</th>"; 
                        echo "<th>วันที่สิ้นสุดการเรียน</th>";
                        echo "<th>ระยะเวลา </th>";
                        echo "<th>ช่องทางการเรียน</th>";
                        echo "<th>รหัสการเชิญ</th>";
                        echo "<th>สถานะ</th>";
                        echo "<th>ชำระเงิน</th>";
                        ...
                        while($row = $result->fetch_assoc()) {
                            echo "<tr>";
                            echo "<td><img src='" . $row["image_course"] . "' alt='" . $row["name_course"] . "'></td>";
                            echo "<td class='td_th_namewidth'>" . $row["name_course"] . "</td>";
                            echo "<td>" . $row["name_std"] . "</td>";
                            echo "<td>" . $row["date_course"] . "</td>";
                            echo "<td>" . $row["date_course_end"] . "</td>";
                            echo "<td>" . $row["day_course"] . "</td>";
                            echo "<td>" . $row["meeting_type"] . "</td>";
                            echo "<td>" . $row["room_course"] . "</td>";
                            // echo "<td>" . $row["status_register"] . "</td>";
                            echo "<td class='" . 
                                ($row['status_register'] == 'รอดำเนินการ' ? 'status-pending' : 
                                ($row['status_register'] == 'ชำระแล้ว' ? 'status-confirmed' : 
                                ($row['status_register'] == 'ชำระไม่สำเร็จ' ? 'status-cancelled' : 
                                ($row['status_register'] == 'ยกเลิก' ? 'status-cancelled' : 'status-unknown')))) . "'>" . 
                                $row['status_register'] . "</td>";
                        
                            echo "<td>". number_format($row["price_course"], 2) . "</td>";
                            echo "<td><button style='border-radius:10px;' onclick=\"showSlip('" . $row["qrCodeUpload_course"] . "')\">ดูหลักฐาน</button></td>";
                            echo "<td><a href='#' onclick='confirmapprove(" . $row["id_register_course"] . ")' class='btn' style='width:85%;'>อนุมัติ</a></td>";
                            echo "<td><a href='meetingroom_edit.php?id=" . $row["id_register_course"] . "' class='edit-button'>แก้ไข</a></td>";
                            // echo "<td><a href='meetingroom_delete.php?id=" . $row["id_room"] . "' class='delete-button'>ลบ</a></td>";
                            echo "<td><a href='#' onclick='confirmDelete(" . $row["id_register_course"] . ")' class='delete-button'>ลบ</a></td>";
                            echo "</tr>";
                        }
                        echo "</tbody>";
                        echo "</table>";
...
function confirmapprove(id_approve) {
        Swal.fire({
            title: 'ยืนยันอนุมัติ?',
            text: "คุณต้องการอนุมัติการลงทะเบียนคอร์สติวนี้ใช่หรือไม่?",
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: 'rgb(77,151,184);',
            cancelButtonColor: '#d33',
            confirmButtonText: 'ใช่, อนุมัติเลย!',
            cancelButtonText: 'ยกเลิก'
        }).then((result) => {
            if (result.isConfirmed) {
                window.location.href = 'register_approve.php?id=' + id_approve;
            }
        });
}
 // ฟังก์ชันเพื่อแสดง SweetAlert พร้อมกับภาพสลิป
 function showSlip(imageUrl) {
        Swal.fire({
            imageUrl: imageUrl,  // URL ของภาพสลิปที่ถูกส่งมาจากฐานข้อมูล
            imageAlt: 'หลักฐานการจ่ายเงิน',
            showCloseButton: true, // เพิ่มปุ่มปิด
            width: '25%', // กำหนดความกว้างของหน้าต่าง
            maxWidth: '400px', // กำหนดความกว้างสูงสุด
            imageWidth: '100%', // ทำให้ภาพขยายให้เต็มความกว้างของหน้าต่าง
        });
    }
</script>
</body>
