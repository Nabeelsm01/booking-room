...
if (isset($_POST['form_room'])) {
    ...
    if (count($errors) == 0) {
        if (isset($_FILES['image_room']) && $_FILES['image_room']['error'] === UPLOAD_ERR_OK) {
            ...
            if (empty($errors)) {
                $sql = "INSERT INTO meetingroom (name_room, detail_room, opacity_room, price_room, room_type, number_room, image_room, address_room) VALUES ('$name_room', '$detail_room', '$opacity_room', '$price_room','$room_type', '$number_room', '$image_room', '$address_room')";
                ...
                if (mysqli_query($conn, $sql)) {
                    ...
                    header('location:meetingroom_add_confirm.php'); // เปลี่ยนเป็นหน้าที่คุณต้องการเปลี่ยนไปเมื่อส่งข้อมูลสำเร็จ
                    exit();
                } else {
    ...
    if (!empty($errors)) {
        ...
        header('location: meetingroom_admin.php');
        exit();
    }
?>