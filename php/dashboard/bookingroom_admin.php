...
background: linear-gradient(158deg, rgba(255,32,32,1) 0%, rgba(172,10,10,1) 100%);"><i class="bi bi-box-arrow-right"></i><span> Logout</span></a>
    ...
    <div class="main-content" id="mainContent">
        <!-- Header -->
        <div class="header">
            <div class="logo" style="font-size:22px;">ระบบจัดการจองห้องประชุม</div>
            <div class="notification">.</div>
        </div>

        <!-- Dashboard Content -->
        <div class="section">
            <h3>สรุปการจองห้องประชุม</h3>
            <div class="card">
                <p>แสดงจำนวนการจองห้องประชุม</p>
                ...
                <?php
                    ...
                    $sql = "SELECT *
                            ...
                            ORDER BY room_booking_summary.id_sum_room DESC";
                    ...
                    if ($result->num_rows > 0) {
                        ...
                        echo "<th>วันเริ่มจอง</th>"; 
                        echo "<th>วันสิ้นสุดจอง</th>";
                        echo "<th>ระยะเวลา </th>";
                        echo "<th>ราคาเต็ม</th>";
                        echo "<th>การชำระเงิน</th>";
                        echo "<th>จ่ายแล้ว</th>";
                        echo "<th>สถานะการจ่าย</th>";
                        echo "<th>หลักฐานการจ่าย</th>";
                        ...
                        while($row = $result->fetch_assoc()) {
                            $modalId = "slipModal_" . $row["id_sum_room"];  // ใช้สำหรับระบุ modal เฉพาะของแต่ละแถว
                            echo "<tr>";
                            ...
                            echo "<td>" . $row["day_book"] . "</td>";
                            echo "<td>". number_format($row["total_pricess"], 2) . "</td>";
                            echo "<td>" . $row["paymentOption"] . "</td>";
                            echo "<td>". number_format($row["totalAmount"], 2) . "</td>";
                            echo "<td class='" . 
                                    ($row['status'] == 'รอดำเนินการ' ? 'status-pending' : 
                                    ($row['status'] == 'ชำระเงินครบแล้ว' ? 'status-confirmed' : 'status-unknown')) . "'>" . 
                                    $row['status'] . "</td>";
                            // ปุ่มเปิด Modal เพื่อดูสลิป
                            echo "<td><button style='border-radius:10px;' onclick=\"showSlip('" . $row["qrCodeUpload"] . "')\">ดูหลักฐาน</button></td>";
                            echo "<td class='" . 
                            ($row['status_summary'] == 'cancelled' ? 'status-cancelled' : 
                            ($row['status_summary'] == 'active' ? 'status-confirmed' : 'status-unknown')) . "'>" . 
                            $row['status_summary'] . "</td>";
                            // echo "<td><a href='bookingroom_approve.php?id=" . $row["id_sum_room"] . "' class='btn' style='width:85%;'>อนุมัติ</a></td>";
                            echo "<td><a href='#' onclick='confirmapprove(" . $row["id_sum_room"] . ")' class='btn' style='width:85%;'>อนุมัติ</a></td>";
                            echo "<td><a href='meetingroom_edit.php?id=" . $row["id_sum_room"] . "' class='edit-button'>แก้ไข</a></td>";
                            // echo "<td><a href='meetingroom_delete.php?id=" . $row["id_room"] . "' class='delete-button'>ลบ</a></td>";
                            echo "<td><a href='#' onclick='confirmDelete(" . $row["id_sum_room"] . ")' class='delete-button'>ลบ</a></td>";
                            echo "</tr>";
                        }
                        echo "</tbody>";
                        echo "</table>";
...
function confirmapprove(id_approve) {
        Swal.fire({
            title: 'ยืนยันอนุมัติ?',
            text: "คุณต้องการอนุมัติการจองห้องประชุมนี้ใช่หรือไม่?",
            icon: 'warning',
            showCancelButton: true,
            ...
            if (result.isConfirmed) {
                window.location.href = 'bookingroom_approve.php?id=' + id_approve;
            }
        });
}
