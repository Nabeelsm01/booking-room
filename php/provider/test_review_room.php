<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $_SESSION['id_room'] = isset($_POST['id_room']) ? $_POST['id_room'] : 0;   
        $id_room = isset($_POST['id_room']) ? intval($_POST['id_room']) : 0; 

        $_SESSION['id_bookingroom'] = isset($_POST['id_bookingroom']) ? $_POST['id_bookingroom'] : 0;   
        $id_bookingroom = isset($_POST['id_bookingroom']) ? intval($_POST['id_bookingroom']) : 0; 
        ...
        $id_room = isset($_SESSION['id_room']) ? intval($_SESSION['id_room']) : 0; 
        $id_bookingroom = isset($_SESSION['id_bookingroom']) ? intval($_SESSION['id_bookingroom']) : 0;
...
<body>
<a href="room_booking_summary.php" class="turn-btn">ย้อนกลับ</a>
    <div class="container">
    ...
    ?>
       <div class="overall-rating">
        ...
        <div class="review-form">
            <h2>ให้รีวิวห้อง</h2>
            <form action="test_submit_review.php" method="post">
                <input type="hidden" name="id_room" value="<?php echo ($id_room); ?>">   <!--ส่งไอดีห้องประชุม value34-->
                ...
                <textarea name="review_room" required></textarea>
                <!-- <input type="hidden" name="id" value="<?php echo ($id); ?>">   ส่งไอดีผู้ใช้ทั่วไป -->
                <input type="hidden" name="id_bookingroom" value="<?php echo ($id_bookingroom); ?>">   <!--ส่งไอดีการจอง-- value416 -->
        ...
        <div class="reviews-container">
            <?php
            if (!empty($id_room)) {
                $sql = "SELECT r.id_review_room, r.rating_room, r.review_room, r.review_date_room, 
            ...
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    ...
                    echo "<h3>ห้อง: " . htmlspecialchars($row['name_room']) . "</h3>";
                    echo "<span class='stars'>";
                    echo "<p style='display:inline; margin-right:10px; color:#b9b9b9;'>" ."คะแนนรีวิว  ". "</p>";
                    echo "<p style='display:inline; margin-right:10px; color:black;'>" . htmlspecialchars($row['rating_room']) . "/5.0</p>";
                    ...
                    echo "<p>" . htmlspecialchars($row['review_room']) . "</p>";
                    echo "</div>";
                    echo "<div class='review-footer'>";
                    if (!empty($row['reviewer_name'])) {
                        echo "<span>โดย: " . htmlspecialchars($row['reviewer_name']) . "</span>";
                    } else {
                        echo "ชื่อผู้รีวิวไม่สามารถแสดงได้";
                    }                    
                    // echo "<span>โดย: " . htmlspecialchars($row['name_lastname']) . "</span>";
                    echo "<span>วันที่: " . htmlspecialchars($row['review_date_room']) . "</span>";
                    echo "</div>";
                    echo "</div>";
                }
            } else {
