    ...
    <div class="allblockcenter">
        <div class="blockcenter">
            <!-- <h1 class="texthead">เว็บให้บริการห้องประชุมและคอร์สติว</h1> -->
             <h2 class="text-content">หน้าสำหรับจัดการอาหารและเครื่องดื่ม</h2>
             <div class="buttonmanage">
                <button class="btn-apply-add" onclick="window.location.href='form_add_food.php'"><i class="bi bi-plus-circle"></i> เพิ่มอาหาร/เครื่องดื่ม</button>
                <button class="btn-apply-edit" onclick="window.location.href='edit_food.php'"><i class="bi bi-pencil-square"></i> แก้ไขอาหาร/เครื่องดื่ม</button>
                <button class="btn-apply-delete" onclick="window.location.href='delete_food.php'"><i class="bi bi-trash3"></i> ลบอาหาร/เครื่องดื่ม</button>
            </div>
            <div class="buttonmanage-2">
            <button class="btn-food" onclick="window.location.href='meeting_food.php'"><i class="bi bi-egg-fried"></i> อาหารหลัก</button>
            <button class="btn-food" onclick="window.location.href='meeting_food2.php'" style="background-color: #dce6ff;"><i class="bi bi-stack-overflow"></i> อาหารว่าง</button>
            <button class="btn-food" onclick="window.location.href='meeting_food3.php'"><i class="bi bi-cup-straw"></i> เครื่องดื่ม</button>
            <button class="btn-food" onclick="window.location.href='meetingroom.php'" style="border:2px solid #90a7dc;"><i class="bi bi-box-arrow-in-up-right"></i> ห้องประชุม</button>
        </div>
        </div>     
               ...
               if (isset($_SESSION['id_provider'])) {
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        echo "<div class='custom-block' data-id='" . $row["id_food"] . "' data-name='" . htmlspecialchars($row["name_food"], ENT_QUOTES) . "' data-detail='" . htmlspecialchars($row["detail_food"], ENT_QUOTES) . "'>";
                        echo "<img src='" . $row["image_food"] . "' alt='" . $row["name_food"] . "'>";
                        echo "<div class='overlay'>ดูรายละเอียดเพิ่มเติม/เลือก</div>";
                        echo "<h2>" . $row["name_food"] . "</h2>";
                        echo "<div class='inline-wrapper'>";
                        echo "<div class='inline_1'>";
                        echo "<p class='details'>รายละเอียด: " . $row["detail_food"] . "</p>";
                        echo "<button class='toggle-details'>..อ่านเพิ่มเติม</button>"; 
                        echo "</div>";
                        echo "<div class='divider'></div>"; 
                        echo "<div class='inline_2'>";
                        echo "<p class='details'>ประเภท: " . $row["type_food"] . " </p>";
                        echo "<p class='details'>ราคา: " . $row["price_food"] . " บาท</p>";
                        echo "</div>";
...
<script>
    ...
    block.addEventListener('click', function() {
        ...
        xhr.open('GET', '/project end/php/provider/get_food_details.php?id=' + foodId, true);
        xhr.onload = function() {
            if (xhr.status >= 200 && xhr.status < 300) {
                try {
                    const data = JSON.parse(xhr.responseText);
                    document.getElementById('custom-modal-img').src = data.image_food; // แสดงรูปภาพ
                    document.getElementById('custom-modal-title').textContent = "ชื่อเมนูอาหาร: " + data.name_food;
                    document.getElementById('custom-modal-details').innerHTML = `
                        <div class="custom-line"></div>
                        <p><strong>รายละเอียด :</strong> ${data.detail_food}</p>
                        <p><i class="bi bi-house"></i><strong> ประเภท :</strong> ${data.type_food}</p>
                        <p><i class="bi bi-cash-coin"></i><strong> ราคา :</strong> ${data.price_food} บาท</p>
                        <div class="custom-line"></div>
