<?php 
    ...
    if (!isset($_SESSION['user_type']) || $_SESSION['user_type'] != 'ติวเตอร์') {
        ...
        header("Location:form_video_student.php");
        exit();
    }
    ...
    if(!empty($id_tutor)){
        $sql = "SELECT course.name_course, course.room_course, tutor.name_lastname_tutor, course.date_course, course.date_course_end
        FROM course 
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                $courses[] = [
                    ...
                    'date_course_end' => $row["date_course_end"]
                ];
...
<h2 class="hh_name">ห้องประชุมออนไลน์สำหรับติวเตอร์</h2>
<div class="container_block">
    <div id="form-container">
        <h2 class="h_name">Enter Room Code (Invite)</h2>
        <form action="jitsi_videomeeting.php" method="GET">
    ...
    if(!empty($courses)){
        foreach ($courses as $course) {
            echo "<div class='list_tutor'> ชื่อคอร์ส: " . $course['name_course'] . ", หมายเลขห้อง: " . $course['number_room'] .  ", ( " . $course['date_course'] ." ถึง ". $course['date_course_end'] .  " )</div><br>";
